...
class CheckoutController extends GetxController {
  ...
  @override
  void onInit() async {
    eProviderSubscription.value = Get.arguments as SalonSubscription;
  ...
  Future loadPaymentMethodsList() async {
    try {
      ...
      var _cashIndex = paymentsList.indexWhere((element) => element.route.toLowerCase() == Routes.CASH);
      if (_cashIndex > -1) {
        // wallet payment method enabled
        // remove existing wallet method
        var _walletPaymentMethod = paymentsList.removeAt(_cashIndex);
  ...
  void paySubscription(SalonSubscription _eProviderSubscription) async {
    try {
