...
class CinetPayController extends GetxController {
  ...
  void getUrl() {
    url.value = _paymentRepository.getCinetpayUrl(eProviderSubscription.value);
    print(url.value);
  }

  void showConfirmationIfSuccess() {
    final _doneUrl = "${Helper.toUrl(Get.find<GlobalService>().baseUrl)}subscription/payments/cinetpay";
    if (url == _doneUrl) {
