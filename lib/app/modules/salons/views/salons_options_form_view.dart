...
class SalonsOptionsFormView extends GetView<SalonsOptionsFormController> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          title: Obx(() {
            return Text(
              controller.isCreateForm() ? controller.salon.value.name ?? '' : controller.availability.value.day ?? '',
          ...
          leading: new IconButton(
            ...
            onPressed: () => Get.offAndToNamed(Routes.SALON, arguments: {'salon': controller.salon.value, 'heroTag': 'option_form_back'}),
          ),
          actions: [
            new IconButton(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                if (controller.isCreateForm())
                  SalonsHorizontalStepperWidget(
                    steps: [
                      SalonsStepWidget(
                        title: Text(
                          ("Salon Availabilty hours".tr).substring(0, min("Salon Availabilty hours".tr.length, 15)),
                        ),
                ...
                Text("Hours".tr, style: Get.textTheme.headline5).paddingOnly(top: 25, bottom: 0, right: 22, left: 22),
                Text("Fill the following details to add availability hour to this salon".tr, style: Get.textTheme.caption).paddingSymmetric(horizontal: 22, vertical: 5),
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                                ...
                                builder: (BuildContext context) {
                                  return SelectDialog(
                                    ...
                                    initialSelectedValue: controller.days.firstWhere(
                                          (element) => element== controller.availability.value.day,
                                      orElse: () => controller.days.isNotEmpty ? controller.days.first : "monday",
                                    ),
                                  );
                                },
                              );
                      ...
                      Obx(() {
                        String selectedDay = controller.days.firstWhere(
                              (element) => element == controller.availability.value.day,
                          orElse: () => controller.days.isNotEmpty ? controller.days.first : "monday",
                        );
                        print(selectedDay);
                        return buildOptionGroup(selectedDay);
                      })
                    ],
                  ),
                ),
  ...
  void _showDeleteDialog(BuildContext context) {
    showDialog<void>(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text(
            "Delete Availability hours".tr,
            style: TextStyle(color: Colors.redAccent),
          ),
          content: SingleChildScrollView(
            child: Column(
              children: <Widget>[
                Text("This hour will removed from the salon".tr, style: Get.textTheme.bodyText1),
              ],
            ),
          ),
          actions: <Widget>[
            ...
            TextButton(
              ...
              onPressed: () {
                Get.back();
                controller.deleteOption(controller.availability.value);
              },
            ),
          ],
        );
