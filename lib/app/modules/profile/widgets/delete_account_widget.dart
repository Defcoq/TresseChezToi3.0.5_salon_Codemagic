        ...
        children: [
          Expanded(
            child: Column(
              ...
              children: [
                Text("Delete your account!".tr, style: Get.textTheme.bodyText2!.merge(TextStyle(color: Colors.redAccent))),
                Text("Once you delete this account, there is no going back. Please be certain.".tr, style: Get.textTheme.caption?.merge(TextStyle(color: Colors.redAccent))),
              ],
            ),
          ),
          SizedBox(width: 10),
          Obx(() {
            final laravelApiClient = Get.find<LaravelApiClient>();
            bool isloading = laravelApiClient.isLoading(task: 'deleteUser') ?? false;
            if (isloading) {
              return Padding(
